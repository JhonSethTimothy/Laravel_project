        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('incoming_items.update', $item->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="serial_number" class="block mt-1 w-full" type="text" name="serial_number" value="{{ old('serial_number', $item->serial_number) }}" required />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="model" :value="__('Model')" />
                            <x-text-input id="model" class="block mt-1 w-full" type="text" name="model" value="{{ old('model', $item->model) }}" required />
                        </div>
                        <div class="mb-4">
                            <x-input-label for="brand" :value="__('Brand')" />
                            <x-text-input id="brand" class="block mt-1 w-full" type="text" name="brand" value="{{ old('brand', $item->brand) }}" required />
                        </div>
                        <div class="mb-4">
